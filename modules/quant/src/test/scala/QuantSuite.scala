...
final class QuantSuite extends FunSuite {
  ...
  test("FixedPoint round-trip ≤ 0.5 LSB") {
    val q = new FixedPointQuantizer(totalBits = 8, fracBits = 4)
    val lsb = 1.0 / (1 << 4)
    val rng = new Random(0)
    (0 until 1000).foreach { _ =>
      val v = rng.nextDouble() * 15.0 - 7.5 // between(-7.5, 7.5)
  ...
  test("Po2 quantizer produces powers of 2") {
    val q = new Po2Quantizer(expBits = 3)
  ...
  test("BFP round-trip relative error < 12.5%") {
    val q = new BFPQuantizer(expBits = 5, manBits = 2)
    val rng = new Random(1)
    (0 until 500).foreach { _ =>
      val base = math.pow(2, rng.nextDouble() * 10.0 - 5.0) // -5.0 to 5.0
      val sign = rng.nextDouble() * 2.0 - 1.0
      val v = base * sign
      if (math.abs(v) >= 1e-12) {
        val dq = q.dequantize(q.quantize(v))
