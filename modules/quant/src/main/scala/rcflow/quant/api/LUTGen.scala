...
object LUTGen {
  ...
  def gen[QB](func: Double => Double, entries: Int, xMin: Double, xMax: Double, q: Aux[QB])(implicit
      ct: ClassTag[QB]
  ): Array[QB] = {
  ...
  def tanh[QB](q: Aux[QB], entries: Int = 1024, range: Double = 4.0)(implicit
      ct: ClassTag[QB]
  ): Array[QB] = {
    gen(math.tanh, entries, -range, range, q)
  }

  def relu[QB](q: Aux[QB], entries: Int = 1024, range: Double = 6.0)(implicit
      ct: ClassTag[QB]
  ): Array[QB] = {
    val f: Double => Double = x =>
      if (x < 0.0) 0.0
      else if (x > range) range
      else x
    gen(f, entries, 0.0, range, q)
  }

  def sign[QB](q: Aux[QB])(implicit ct: ClassTag[QB]): Array[QB] = {
