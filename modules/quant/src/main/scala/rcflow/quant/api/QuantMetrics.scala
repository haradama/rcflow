...
object QuantMetrics:
  ...
  inline private def mean(a: DenseVector[Double]): Double = sum(a) / a.length

  inline private def mseVec(orig: DenseVector[Double], recon: DenseVector[Double]): Double =
    val diff = orig - recon
  ...
  inline private def mseMat(orig: DenseMatrix[Double], recon: DenseMatrix[Double]): Double =
    val diff = orig - recon
